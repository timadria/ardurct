...
void tdRefresh(uint8_t throttle, uint8_t yaw, uint8_t pitch, uint8_t roll, uint8_t adjust) {
    if (throttle != tdThrottle) {
        tdThrottle = throttle;
        lcdT.drawProgressBar(2, THROTTLE_LINE, 12, throttle, ON);
    }
    if (yaw != tdYaw) {
        tdYaw = yaw;
        lcdT.drawProgressBar(2, YAW_LINE, 12, yaw, OFF);
    }
    if (pitch != tdPitch) {
        tdPitch = pitch;
        lcdT.drawProgressBar(2, PITCH_LINE, 12, pitch, OFF);
    }
    if (roll != tdRoll) {
        tdRoll = roll;
        lcdT.drawProgressBar(2, ROLL_LINE, 12, roll, OFF);
    }
    if (adjust != tdAdjust) {
        tdAdjust = adjust;
        lcdT.drawProgressBar(2, ADJUST_LINE, 12, adjust, OFF);
    }
}
