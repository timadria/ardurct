...
public class MacroDrawing extends ScreenHAL {
	...
	public final static int SCREEN_MACRO_PARAM_RADIUS = 0;
	...
	public void executeMacro(String macro) {
		...
		executeMacro(buffer, 0, 0, 1, 1, true, true, true);
	}
	...
	void _executeMacroCommand(macroCommand_t mc, int x, int y, int scaleMul, int scaleDiv, boolean isScaleThickness) {
		...
		if (group == SCREEN_MACRO_CMD_GROUP_PRESET) {
			...
			else if (mc.cmd == SCREEN_MACRO_CMD_PRESET_THICKNESS) _mThickness = mc.param[SCREEN_MACRO_PARAM_THICKNESS];
	...
	int _parseHexColor(int s[], int n, macroCommand_t mc) {
		int i = n;
		mc.color = 0;
		// remove front spaces
		while ((s[i] != 0) && (s[i] <= ' ')) i++;
	...
	int _parseParameter(int s[], int n, macroCommand_t mc, int paramId) {
		int i = n;
	...
	int _compressMacroCommand(macroCommand_t mc, int buffer[], int bufferPtr) {
		...
		if (group == SCREEN_MACRO_CMD_GROUP_PRESET) {
			if ((mc.cmd == SCREEN_MACRO_CMD_PRESET_FOREGROUND) || (mc.cmd == SCREEN_MACRO_CMD_PRESET_BACKGROUND)) { 
				...
				buffer[i++] = mc.param[SCREEN_MACRO_PARAM_THICKNESS] & 0x0FF;
			} else if (mc.cmd == SCREEN_MACRO_CMD_PRESET_FONT) {
