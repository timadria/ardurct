...
public class RealTimeClock  {
	...
	public final static int RTC_MILLIS_TO_SEC = 10;

	private final static int RTC_ALARM_MAX_SNOOZE = 0xFF;
	private final static int RTC_ALARM_STOPPED = (-RTC_ALARM_MAX_SNOOZE-1);
	private final static int RTC_ALARM_RING_START = 0x01;
	...
	public RealTimeClock(int year, int month, int day, int dayOfWeek, int hour, int minute, int second) {
		for (int i=0; i<7; i++) _alarm[i] = new rtcAlarm_t();
	    set(year, month, day, dayOfWeek, hour, minute, second);
	}

	void set(int year, int month, int day, int dayOfWeek, int hour, int minute, int second) {
	    _year = year;
	    _month = month % 13;
