...
public class ArduRCT_Graphics
implements IFontBitmaps, IGraphicsDefines {
	...
	public void begin(int foregroundColor, int backgroundColor, int fontSize, boolean fontBold, boolean fontOverlay) {
		...
		if (backgroundColor != BLACK) fillRectangle(0, 0, _width, _height, backgroundColor, true);
	}

	public void setupBacklight(int backlightPin) { 	
	}
	...
	public void getRotatedXY(int x[], int y[]) {
		...
		if (_rotation == GRAPHICS_ROTATION_90) {
			x[0] = getWidthImpl() - 1 - Y;
			y[0] = X;
		} else if (_rotation == GRAPHICS_ROTATION_180) {
			x[0] = getWidthImpl() - 1 - X;
			y[0] = getHeightImpl() - 1 - Y;
		} else if (_rotation == GRAPHICS_ROTATION_270) {
			x[0] = Y;
			y[0] = getHeightImpl() - 1 - X;
		} 
	}
	...
	protected void setFont(int size, boolean bold, boolean overlay) {
		_fontSize = size;
		_isFontBold = bold;
	...
	public void drawBigDigit(int d, int x, int y, int width, int height, int color, int thickness, int style) {
		...
		if ((DIGIT_SEGMENTS[d] & 0x01) != 0) {
			...
			if (style >= GRAPHICS_STYLE_NORMAL) {
				fillCorner(x+thickness-1, y1+thickness-1, thickness, GRAPHICS_POSITION_NW, color);
				fillCorner(x+width-thickness, y1+thickness-1, thickness, GRAPHICS_POSITION_NE, color);
			}
		}
		if ((DIGIT_SEGMENTS[d] & 0x02) != 0) {
			...
			if (style >= GRAPHICS_STYLE_NORMAL) {
				fillCorner(x+thickness-1, y1+thickness-1, thickness, GRAPHICS_POSITION_NW, color);
				fillCorner(x+thickness-1, y1+h+thickness, thickness, GRAPHICS_POSITION_SW, color);
			}
			if (style >= GRAPHICS_STYLE_ADVANCED) {
				if ((DIGIT_SEGMENTS[d] & 0x01) != 0) fillCorner(x+thickness, y1+thickness, t2, GRAPHICS_POSITION_SE, color);
				if ((DIGIT_SEGMENTS[d] & 0x40) != 0) fillCorner(x+thickness, y1+h+thickness-1, t2, GRAPHICS_POSITION_NE, color);
			}
		}
		if ((DIGIT_SEGMENTS[d] & 0x04) != 0) {
			...
			if (style >= GRAPHICS_STYLE_NORMAL) {			
				fillCorner(x+thickness-1, y2+thickness-1, thickness, GRAPHICS_POSITION_NW, color);
				fillCorner(x+thickness-1, y2+h+thickness, thickness, GRAPHICS_POSITION_SW, color);
				if (DIGIT_SEGMENTS[d] == 0x3F || DIGIT_SEGMENTS[d] == 0x5F) fillRectangle(x, y2, thickness, thickness, color);
			}
			if (style >= GRAPHICS_STYLE_ADVANCED) {
				if ((DIGIT_SEGMENTS[d] & 0x40) != 0) fillCorner(x+thickness, y2+thickness, t2, GRAPHICS_POSITION_SE, color);
				if ((DIGIT_SEGMENTS[d] & 0x08) != 0) fillCorner(x+thickness, y2+h+thickness-1, t2, GRAPHICS_POSITION_NE, color);
			}
		}
		if ((DIGIT_SEGMENTS[d] & 0x08) != 0) {
			...
			if (style >= GRAPHICS_STYLE_NORMAL) {	
				fillCorner(x+thickness-1, y3, thickness, GRAPHICS_POSITION_SW, color);
				fillCorner(x+width-thickness, y3, thickness, GRAPHICS_POSITION_SE, color);
			}
		}
		if ((DIGIT_SEGMENTS[d] & 0x10) != 0) {
			...
			if (style >= GRAPHICS_STYLE_NORMAL) {
				fillCorner(x+width-thickness, y2+thickness-1, thickness, GRAPHICS_POSITION_NE, color);
				fillCorner(x+width-thickness, y2+h+thickness, thickness, GRAPHICS_POSITION_SE, color);
			}
			if (style >= GRAPHICS_STYLE_ADVANCED) {
				if ((DIGIT_SEGMENTS[d] & 0x40) != 0) fillCorner(x+width-thickness-1, y2+thickness, t2, GRAPHICS_POSITION_SW, color);
				if ((DIGIT_SEGMENTS[d] & 0x08) != 0) fillCorner(x+width-thickness-1, y2+h+thickness-1, t2, GRAPHICS_POSITION_NW, color);
			}
		}
		if ((DIGIT_SEGMENTS[d] & 0x20) != 0) {
			...
			if (style >= GRAPHICS_STYLE_NORMAL) {
				fillCorner(x+width-thickness, y1+thickness-1, thickness, GRAPHICS_POSITION_NE, color);
				fillCorner(x+width-thickness, y1+h+thickness, thickness, GRAPHICS_POSITION_SE, color);
			...
			if (style >= GRAPHICS_STYLE_ADVANCED) {
				if ((DIGIT_SEGMENTS[d] & 0x01) != 0) fillCorner(x+width-thickness-1, y1+thickness, t2, GRAPHICS_POSITION_SW, color);
				if ((DIGIT_SEGMENTS[d] & 0x40) != 0) fillCorner(x+width-thickness-1, y1+h+thickness-1, t2, GRAPHICS_POSITION_NW, color);
			}
		}
	...
	public void fillCorner(int x, int y, int size, int type, int color) {
		...
		for (int i=0; i<size; i++) {
			if ((type & GRAPHICS_POSITION_NW) != 0) drawHorizontalLine(x-size+i+1, x, y-i, color, 1);
			if ((type & GRAPHICS_POSITION_NE) != 0) drawHorizontalLine(x, x+size-i-1, y-i, color, 1);
			if ((type & GRAPHICS_POSITION_SE) != 0) drawHorizontalLine(x, x+size-i-1, y+i, color, 1);
			if ((type & GRAPHICS_POSITION_SW) != 0) drawHorizontalLine(x-size+i+1, x, y+i, color, 1);
		}
	}
	...
	protected void fillRectangle(int x, int y, int width, int height, int color, boolean grabAndReleaseBus) {
		if ((width == 0) || (height == 0)) return;
		if (grabAndReleaseBus) _grabBus();
	...
	void fillScreen(int color, boolean grabAndReleaseBus) {
		fillRectangle(0, 0, _width, _height, color, grabAndReleaseBus);
	}
