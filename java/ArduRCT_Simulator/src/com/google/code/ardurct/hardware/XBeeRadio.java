...
public class XBeeRadio extends HardwareSerial implements Runnable {
	...
	public XBeeRadio(String anId, String aSHSL, int aHardwareBaudrate) {
		super("");
		id = anId;
		shsl = aSHSL;
	...
	public void connect(XBeeRadio aRemote) {
		remote = aRemote;
	}
	...
	public void setDHDL(String aDhdl) {
		dhdl = aDhdl;
		updateConfiguration();
	}
	...
	public void run() {
		while (true) {
			try {
				if (toSendQueue.getText().length() - processedLength > 0) {
					if (hardwareBaudrate != getBaudrate()) {
						...
						String cmdText = toSendQueue.getText().substring(processedLength).toUpperCase();
						if (isCommandMode) processCommandList(cmdText);
							...
							if (indOf != -1) {
								...
								for (int i=0; i<indOf; i++) {
									char c = (char)read();
									if (remote != null && remote.shsl.equals(dhdl)) remote.print(c);
									processedLength++;
								}
	...
	private boolean processCommand(String cmd) {	
		if (cmd.startsWith("CN")) {
			...
			if (cmd.length() > 2) {
				dhdl = cmd.substring(2) + dhdl.substring(8, 16);
				print("OK\n");
				updateConfiguration();
			} else print(dhdl.substring(0, 8) + "\n"); 
		} else if (cmd.startsWith("DL")) {
			if (cmd.length() > 2) {
				dhdl = dhdl.substring(0, 8) + cmd.substring(2);
				print("OK\n");
				updateConfiguration();
